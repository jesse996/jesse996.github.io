...
在单节点上，事务提交非常依赖于数据持久写入磁盘的顺序关系：先写入数据，然后再提交记录。事务提交（或中止）的关键点在于磁盘完成日志记录的时刻：在完成日志记录写之前如果发生了崩溃，则事务需要中止否则事务被提交。
...
因此，实际有两轮投票：首先投票决定谁是主节点，然后是对主节点的提议投票。关键一点是，参与两轮的 quoram 必须有重叠。
